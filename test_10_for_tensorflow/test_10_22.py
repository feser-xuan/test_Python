...
def init_sample():
    ...
    Xn = data[['weather','music']].values
    # Xn = pd.read_csv('config/train.csv', usecols=[2]).values
    # Xn = pd.read_csv('config/train.csv', usecols=[3]).values
    # Xn = pd.read_csv('config/train.csv', usecols=[4]).values
    dataLen = len(Xn)
    return Xn, dataLen
